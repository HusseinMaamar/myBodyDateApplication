...
public class User {
	...
	public User(String mail, String phone, String pseudonym, String password, String city, GeoPoint location,
			String dynamicGeolocation, LocalDateTime dateOfCreation, boolean loveCoatch, boolean multiProfil,
			boolean subscriber, Role role, String firstname, String lastname, String dateOfBirth, Category category,
			String size, List<String> languages, byte[] picture, byte[] voicePrint, byte[] proofOfIdentity,
			Gender gender, Situation situation, SexualOrientation sexualOrientation, Gender genderWanted,
			List<Relation> relations, List<Objective> objectives, List<Affinity> affinities, LifeStyle lifeStyle,
			List<LifeStyle2> lifeStyle2) {
		this.mail = mail;
		this.phone = phone;
		...
		this.userId = String.format("%04d%02d%02d%s", dateOfCreation.getYear(), dateOfCreation.getMonthValue(),
				dateOfCreation.getDayOfMonth(), UUID.randomUUID().toString());
